// ✅ Server Component — next/image للـ background
...
export const LibraryPage = async () => {
  ...
  return (
    <section className="relative flex flex-col justify-center items-center px-6 min-h-screen">
      {/* ✅ next/image بدل background-image CSS */}
      <div className="absolute inset-0">
        <Image
          src="/images/backgrounds/backgroundlibarary.png"
          alt=""
          fill
      ...
      <div className="relative z-10 w-full max-w-7xl text-center py-20 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sections.map(item => (
            <div
              key={item.id}
              className="bg-white/85 p-8 rounded-2xl shadow-lg hover:shadow-2xl transition-all 
              transform hover:-translate-y-2 border border-gray-200 backdrop-blur-md text-center"
            >
